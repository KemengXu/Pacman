...
class Pacman(GameCharacter):
    ...
    def __init__(self, maze, game_controller):
        ...
        self.mouth_speed = 5
        self.velocity = 3
        self.WALL_TOLERANCE = 20  # changed, because too insensitive to control
    ...
    def update(self):
        ...
        # because there are dots at the beginning and ending on each ends
        # but they are actually the same dot! We should delete both when
        # packman reaches the side border!
        if self.x == self.maze.WIDTH or self.x == 0:
            self.maze.eat_dots(0, self.y)
            self.maze.eat_dots(self.maze.WIDTH, self.y)
        if self.y == self.maze.HEIGHT or self.y == 0:
            self.maze.eat_dots(self.x, 0)
            self.maze.eat_dots(self.x, self.maze.HEIGHT)
        # END CODE CHANGES
